...
#define ANTON_ARRAY_MIN_ALLOCATION_SIZE (static_cast<i64>(64))
  ...
  public:
    ...
    template<typename Input_Iterator>
    Array(allocator_type const& allocator, Range_Construct_Tag,
    ...
    // back
    // Accesses the last element of the array. The behaviour is undefined when
    // the array is empty.
    //
    [[nodiscard]] T& back();
    ...
    // resize
    // Resizes the array allocating additional memory if n is greater than
    // capacity.
    ...
    // assign
    // Overwrite the contents of the array with elements from the range
    // [first, last[.
    //
    // Parameters:
    ...
    // insert
    // Constructs an object directly into array at position avoiding copies or
    // moves.
    //
    // Parameters:
    // position - iterator to the insert position. Must be a valid iterator.
    ...
    //
    template<typename... Args>
    iterator insert(Variadic_Construct_Tag, const_iterator position,
                    Args&&... args);

    // insert
    // Constructs an object directly into array at position avoiding copies or
    // moves. position must be an index greater than or equal 0 and less than or
    // equal size.
    //
    // Returns:
    // iterator to the inserted element.
    ...
    // insert
    // Insert a range of elements into array at position. position must be a
    // valid iterator.
    //
    // Returns:
    ...
    //
    template<typename Input_Iterator>
    iterator insert(const_iterator position, Input_Iterator first,
                    Input_Iterator last);

    // insert
    // Insert a range of elements into array at position. position must be an
    // index greater than or equal 0 and less than or equal size.
    //
    // Returns:
    // iterator to the first of the inserted elements.
    //
    template<typename Input_Iterator>
    iterator insert(size_type position, Input_Iterator first,
                    Input_Iterator last);

    // insert_unsorted
    // Inserts an element into array by moving the object at position to the end
    // of the array and then copying value into position. position must be a
    // valid iterator.
    //
    // Returns:
    // iterator to the inserted element.
    ...
    // insert_unsorted
    // Inserts an element into array by moving the object at position to the end
    // of the array and then copying value into position. position must be an
    // index greater than or equal 0 and less than or equal size.
    //
    // Returns:
    // iterator to the inserted element.
    ...
    // reset_lose_memory
    // Unilateral reset of the array. No destructors are invoked, deallocation
    // does not occur. The container is reset to the initial empty state.
    void reset_lose_memory();

    // swap
    // Exchanges the contents of the two arrays without copying, moving or
    // swapping the individual elements. Exchanges the allocators.
    //
    // Parameters:
    ...
    {
      // We do not follow the C++ Standard in its complex ways to swap
      // containers and always swap both the allocator and the memory since it
      // is a common expectation of the users, is much faster than copying all
      // the elements and does not break the container or put it in an invalid
      // state.
      using anton::swap;
...
namespace anton {
  ...
  template<typename T>
  Array<T>::Array(allocator_type const& allocator, size_type const n)
  ...
  template<typename T>
  Array<T>::Array(size_type n, value_type const& value)
    : Array(allocator_type(), n, value)
  {
  }

  template<typename T>
  Array<T>::Array(allocator_type const& allocator, size_type n,
                  value_type const& value)
    : _allocator(allocator)
  {
    _capacity = math::max(ANTON_ARRAY_MIN_ALLOCATION_SIZE, n);
  ...
  template<typename T>
  Array<T>::Array(Reserve_Tag, size_type const n)
    : Array(allocator_type(), reserve, n)
  {
  }

  template<typename T>
  Array<T>::Array(allocator_type const& allocator, Reserve_Tag,
                  size_type const n)
    : _allocator(allocator)
  {
    _capacity = math::max(ANTON_ARRAY_MIN_ALLOCATION_SIZE, n);
  ...
  template<typename T>
  Array<T>::Array(allocator_type const& allocator, Array const& other)
    : _allocator(allocator), _capacity(other._capacity)
  {
    if(_capacity > 0) {
      _data = allocate(_capacity);
  ...
  Array<T>::Array(Array&& other)
    : _allocator(ANTON_MOV(other._allocator)), _capacity(other._capacity),
  ...
  template<typename T>
  Array<T>::Array(allocator_type const& allocator, Array&& other)
    : _allocator(allocator), _capacity(other._capacity), _size(other._size)
  {
  ...
  template<typename T>
  template<typename Input_Iterator>
  Array<T>::Array(Range_Construct_Tag, Input_Iterator first,
                  Input_Iterator last)
    : Array(allocator_type(), range_construct, ANTON_MOV(first),
            ANTON_MOV(last))
  {
  }
  ...
  template<typename T>
  template<typename Input_Iterator>
  Array<T>::Array(allocator_type const& allocator, Range_Construct_Tag,
                  Input_Iterator first, Input_Iterator last)
    : _allocator(allocator)
  {
    // TODO: Use distance?
  ...
  template<typename T>
  template<typename... Args>
  Array<T>::Array(Variadic_Construct_Tag, Args&&... args)
    : Array(allocator_type(), variadic_construct, ANTON_FWD(args)...)
  {
  }

  template<typename T>
  template<typename... Args>
  Array<T>::Array(allocator_type const& allocator, Variadic_Construct_Tag,
                  Args&&... args)
    : _allocator(allocator)
  {
    _capacity = math::max(ANTON_ARRAY_MIN_ALLOCATION_SIZE,
                          static_cast<size_type>(sizeof...(Args)));
    _data = allocate(_capacity);
  ...
  Array<T>& Array<T>::operator=(Array const& other)
  {
    anton::destruct_n(_data, _size);
    _size = 0;
    // We do not shrink the container to fit as it is faster that way - we avoid
    // an allocation after all! Shrinking may be requested by the user
    // explicitly.
  ...
  void Array<T>::ensure_capacity(size_type requested_capacity)
  {
    if(requested_capacity > _capacity) {
      size_type new_capacity =
        (_capacity > 0 ? _capacity : ANTON_ARRAY_MIN_ALLOCATION_SIZE);
  ...
  template<typename T>
  template<typename... Args>
  auto Array<T>::insert(Variadic_Construct_Tag, const_iterator position,
                        Args&&... args) -> iterator
  {
    size_type const offset = static_cast<size_type>(position - begin());
  ...
  template<typename T>
  template<typename... Args>
  auto Array<T>::insert(Variadic_Construct_Tag, size_type const position,
                        Args&&... args) -> iterator
  {
    if constexpr(ANTON_ITERATOR_DEBUG) {
    ...
    if(_size == _capacity || position != _size) {
      if(_size != _capacity) {
        ...
        anton::move_backward(_data + position, _data + _size - 1,
                             _data + _size);
        anton::construct(_data + position, ANTON_FWD(args)...);
        _size += 1;
      } else {
        i64 const new_capacity =
          (_capacity > 0 ? _capacity * 2 : ANTON_ARRAY_MIN_ALLOCATION_SIZE);
        ...
        moved += 1;
        anton::uninitialized_move(_data + position, _data + _size,
                                  new_data + moved);
      ...
      // Quick path when position points to end and we have room for one more
      // element.
  ...
  template<typename T>
  template<typename Input_Iterator>
  auto Array<T>::insert(const_iterator position, Input_Iterator first,
                        Input_Iterator last) -> iterator
  {
    size_type const offset = static_cast<size_type>(position - begin());
  ...
  template<typename T>
  template<typename Input_Iterator>
  auto Array<T>::insert(size_type position, Input_Iterator first,
                        Input_Iterator last) -> iterator
  {
    if constexpr(ANTON_ITERATOR_DEBUG) {
      ANTON_FAIL(position >= 0 && position <= _size, "index out of bounds");
    ...
    if(first != last) {
      ...
      ANTON_ASSERT(new_elems > 0,
                   "the difference of first and last must be greater than 0");
      if(_size + new_elems <= _capacity && position == _size) {
        // Quick path when position points to end and we have room for new_elems
        // elements.
        ...
        if(_size + new_elems <= _capacity) {
          ...
          // When new_elems < total_elems, we have to unititialized_move
          // min(total_elems, new_elems) and move_backward the rest because the
          // target range will overlap the source range.
          i64 const elems_outside = math::min(total_elems, new_elems);
          i64 const elems_inside = total_elems - elems_outside;
          // We move the 'outside' elements to _size unless position + new_elems
          // is greater than _size.
          i64 const target_offset = math::max(position + new_elems, _size);
          anton::uninitialized_move_n(_data + position + elems_inside,
                                      elems_outside, _data + target_offset);
          anton::move_backward(_data + position,
                               _data + position + elems_inside,
                               _data + position + new_elems + elems_inside);
          // We always have to destruct at most total_elems and at least
          // new_elems if we attempt to destruct more than total_elems, we will
          // call destruct on uninitialized memory.
  ...
  template<typename T>
  auto Array<T>::insert_unsorted(const_iterator position,
  ...
  template<typename T>
  auto Array<T>::insert_unsorted(size_type position,
                                 value_type const& value) -> iterator
  {
    if constexpr(ANTON_ITERATOR_DEBUG) {
      ANTON_FAIL(position >= 0 && position <= _size, "index out of bounds");
  ...
  {
    if constexpr(ANTON_ITERATOR_DEBUG) {
      ANTON_FAIL(iter - _data >= 0 && iter - _data <= _size,
  ...
  //   template<typename T>
  //   auto Array<T>::erase_unsorted(const_iterator first,
  ...
  {
    if constexpr(ANTON_ITERATOR_DEBUG) {
      ANTON_FAIL(first - _data >= 0 && first - _data <= _size,
                 "iterator out of bounds");
      ANTON_FAIL(last - _data >= 0 && last - _data <= _size,
                 "iterator out of bounds");
    }

    if(first != last) {
      iterator pos = anton::move(const_cast<value_type*>(last), end(),
  ...
  {
    void* mem = _allocator.allocate(size * static_cast<isize>(sizeof(T)),
                                    static_cast<isize>(alignof(T)));
  ...
  {
    _allocator.deallocate(mem, size * static_cast<isize>(sizeof(T)),
                          static_cast<isize>(alignof(T)));
  }
} // namespace anton
