...
namespace anton {
  ...
  {
    String dialog_text = format(
      u8"Assertion failed:\n{}\nin file {} on line {}\n\nStack trace:\n"_sv,
      message, file, line);
    Array<stacktrace::Frame_Record*> frame_records =
      stacktrace::capture_call_stack();
    ...
    // If the above methods fail (although they shouldn't), we enter an infinite
    // loop to never return.
    while(true) {}
  }
} // namespace anton
