...
namespace anton {
  ...
  // Compare must provide a templated on 2 parameters static member variable
  // called compare that can be converted to bool.
  //
  ...
  private:
    ...
    private:
      ...
      struct _Type_Swap<_TS_List, _TS_I, _TS_J,
                        Integer_Sequence<u64, Indices...>> {
        using type =
          Type_List<tuple_element<(Indices != _TS_I && Indices != _TS_J)
                                    ? Indices
                                    : (Indices == _TS_I ? _TS_J : _TS_I),
                                  _TS_List>...>;
      };

    public:
      using type =
        typename _Type_Swap<TS_List, TS_I, TS_J,
                            make_integer_sequence<u64, tuple_size<List>>>::type;
    };

    template<u64 I, u64 J, u64 Least, u64 List_Size, typename TS_List,
             typename TS_Compare>
    struct Inner_Loop {
    private:
      static constexpr bool compare_res =
        TS_Compare::template compare<tuple_element<Least, TS_List>,
                                     tuple_element<J, TS_List>>;

    public:
      using type = typename Inner_Loop<I, J + 1, compare_res ? Least : J,
                                       List_Size, TS_List, TS_Compare>::type;
    };

    template<u64 I, u64 Least, u64 List_Size, typename TS_List,
             typename TS_Compare>
    ...
    struct Outer_Loop {
      using iteration_res =
        typename Inner_Loop<I, I + 1, I, List_Size, TS_List, TS_Compare>::type;
      using type =
        typename Outer_Loop<I + 1, List_Size, iteration_res, TS_Compare>::type;
    };
