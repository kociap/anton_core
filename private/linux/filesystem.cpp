...
namespace anton::fs {
  ...
  // - fts essentially exposes 2 APIs - fts_read and fts_children. The behaviour
  //   of those differs. While using fts_read, fts_path will also include
  //   fts_name, but when using fts_children that is not the case anymore.
  // - fts_children will list only one entry if fts_read has not been called
  //   behaving as if it were called from a directory above and filtering all
  //   other entries.
  // - fts can identify directories without stat (FTS_NOSTAT), but it cannot
  //   identify files without stat.
  ...
  Array<String> enumerate_directories(String_View const path)
  {
    ...
    FTS* const fts =
      fts_open(paths, FTS_PHYSICAL | FTS_NOSTAT | FTS_NOCHDIR, nullptr);
    // We call fts_read to "descend into the directory" specified to fts_open.
    // Otherwise fts_children will give us a single entry which is the path.
