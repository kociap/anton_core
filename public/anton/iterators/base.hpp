...
namespace anton {
  ...
  template<typename T, typename Category>
  inline constexpr bool is_iterator_category =
    Is_Iterator_Category<T, Category>::value;

  // Is_Iterator_Wrapper
  // Checks whether a type is an iterator wrapper. Relies on the class daclaring
  // wrapped_iterator_type alias.
  //
  template<typename T, typename = void>
  struct Is_Iterator_Wrapper: False_Type {};

  template<typename T>
  struct Is_Iterator_Wrapper<T, void_sink<typename T::wrapped_iterator_type>>
    : True_Type {};
