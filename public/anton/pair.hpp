...
namespace anton {
  ...
  public:
    ...
    constexpr Pair(U1&& u1, U2&& u2)
      : first(ANTON_FWD(u1)), second(ANTON_FWD(u2))
    {
    }
    template<typename U1, typename U2>
    constexpr Pair(Pair<U1, U2> const& pair)
      : first(pair.first), second(pair.second)
    {
    }
    template<typename U1, typename U2>
    constexpr Pair(Pair<U1, U2>&& pair)
      : first(ANTON_MOV(pair.first)), second(ANTON_MOV(pair.second))
    {
    }
  ...
  template<typename T1, typename T2>
  [[nodiscard]] constexpr bool operator==(Pair<T1, T2> const& a,
                                          Pair<T1, T2> const& b)
  {
  ...
  struct Tuple_Element<I, Pair<T1, T2>> {
    static_assert(I == 0 || I == 1, "Pair has only 2 elements");
  ...
  template<typename T1, typename T2>
  constexpr enable_if<is_swappable<T1> && is_swappable<T2>>
  ...
  {
    static_assert(N == 0 || N == 1, "Pair has only 2 elements");
    if constexpr(N == 0) {
      return p.first;
    } else {
      return p.second;
  ...
  template<usize N, typename T1, typename T2>
  [[nodiscard]] constexpr tuple_element<N, Pair<T1, T2> const>&
  get(Pair<T1, T2> const& p)
  {
    static_assert(N == 0 || N == 1, "Pair has only 2 elements");
    if constexpr(N == 0) {
      return p.first;
  ...
  [[nodiscard]] constexpr T1& get(Pair<T1, T2>& p)
  {
    static_assert(
      !is_same<T1, T2>,
      "Type based get may not be called with Pair with the same types.");
    return p.first;
  }
...
namespace std {
  template<typename T1, typename T2>
  struct tuple_size<anton::Pair<T1, T2>>
    : anton::Tuple_Size<anton::Pair<T1, T2>> {};

  template<typename T1, typename T2>
  struct tuple_size<anton::Pair<T1, T2> const>
    : anton::Tuple_Size<anton::Pair<T1, T2> const> {};

  template<anton::usize I, typename T1, typename T2>
  struct tuple_element<I, anton::Pair<T1, T2>>
    : anton::Tuple_Element<I, anton::Pair<T1, T2>> {};

  template<anton::usize I, typename T1, typename T2>
  struct tuple_element<I, anton::Pair<T1, T2> const>
    : anton::Tuple_Element<I, anton::Pair<T1, T2> const> {};
} // namespace std
