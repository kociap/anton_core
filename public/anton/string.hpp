...
namespace anton {
  ...
  //
  // Does not provide begin/end functions, but instead exposes 2 functions -
  // bytes and chars. Both return proxy classes which have begin/end functions
  // that return iterators over bytes and code points respectively.
  //
  // Notes:
  // operator[] is not implemented because UTF-8 doesn't allow us to index in
  // constant-time.
  //
  // TODO: SSO
  ...
  public:
    ...
    //
    // length is the number of bytes to be used to construct String. If length
    // is -1, the function will convert all characters up until and including
    // the null-terminator.
    //
    [[nodiscard]] static String from_utf32(char32 const* string,
                                           i64 length = -1);
    ...
    // the null-terminator.
    //
    [[nodiscard]] static String from_utf16(char16 const* string,
                                           i64 length = -1);
  ...
  public:
    ...
    // ensure_capacity
    // Allocates at least requested_capacity + 1 (for null-terminator) bytes of
    // storage. Does nothing if requested_capacity is less than capacity().
    //
    void ensure_capacity(size_type requested_capacity);

    // ensure_capacity_exact
    // Allocates exactly requested_capacity (null-terminator is not accounted
    // for) bytes of storage. Does nothing if requested_capacity is less than
    // capacity().
    //
  ...
  // Concatenated string.
  //
  [[nodiscard]] String concat(Memory_Allocator* allocator,
                              Slice<String_View const> strings);
  ...
  [[nodiscard]] enable_if<is_constructible<String_View, T1&&>, String>
  concat(T1&& first, T2&& second, T&&... rest)
  {
    String_View strings[2 + sizeof...(rest)] = {
      String_View(first), String_View(second), String_View(rest)...};
  ...
  //
  template<typename T1, typename T2, typename... T>
  [[nodiscard]] String concat(Memory_Allocator* const allocator, T1&& first,
                              T2&& second, T&&... rest)
  {
    String_View strings[2 + sizeof...(rest)] = {
      String_View(first), String_View(second), String_View(rest)...};
  ...
  //
  [[nodiscard]] String replace(String_View string, String_View pattern,
  ...
  // A new string with all occurrences of pattern replaced with replacement.
  //
  [[nodiscard]] String replace(Memory_Allocator* allocator, String_View string,
                               String_View pattern, String_View replacement);
  ...
  struct Equal_Compare<String> {
    ...
    [[nodiscard]] constexpr bool operator()(String_View const lhs,
