        ...
        public:
            ...
            [[nodiscard]] value_type* operator->() const {
                if constexpr(ANTON_ITERATOR_DEBUG) {
                    ANTON_FAIL(*_states == State::active, u8"Dereferencing invalid Flat_Hash_Map iterator.");
                }
    ...
    void Flat_Hash_Map<Key, Value, Hash, Key_Compare>::erase(const_iterator pos) {
        if constexpr(ANTON_ITERATOR_DEBUG) {
            ANTON_FAIL(pos._states >= _states && pos._states < _states + _capacity,
                       u8"Flat_Hash_Map::erase(const_iterator): Attepmting to erase an iterator outside the container.");
            ANTON_FAIL(*pos._states == State::active,
                       u8"Flat_Hash_Map::erase(const_iterator): Attempting to erase an iterator that doesn't point to a valid object.");
        }
