...
namespace anton {
  namespace detail {
    ...
    [[nodiscard]] Return zip_dereference(Tuple<Ts...> const& tuple,
                                         integer_sequence<u64, Indices...>)
    {
      return Return(
        (*static_cast<Tuple_Child<Indices, Ts> const&>(tuple).value)...);
    }

    template<typename... Ts, u64... Indices>
    [[nodiscard]] bool zip_equal(Tuple<Ts...> const& lhs,
                                 Tuple<Ts...> const& rhs,
                                 integer_sequence<u64, Indices...>)
    {
      return ((static_cast<Tuple_Child<Indices, Ts> const&>(lhs).value ==
               static_cast<Tuple_Child<Indices, Ts> const&>(rhs).value) ||
              ...);
    }
  } // namespace detail
  ...
  struct Zip_Iterator {
    ...
    using value_type =
      Tuple<typename Iterator_Traits<Iterators>::value_type...>;
    ...
    explicit Zip_Iterator(Iterators const&... iterators)
      : _iterators(iterators...)
    {
    }
    explicit Zip_Iterator(Iterators&&... iterators)
      : _iterators(ANTON_MOV(iterators)...)
    {
    }

    // TODO: Implement the below functions with a flat function call instead of
    // apply. See zip_dereference.

    [[nodiscard]] Tuple<Iterators const&...> base() const
    {
      return apply(_iterators,
                   [](auto&&... iterators) -> Tuple<Iterators const&...> {
                     return {iterators...};
                   });
    }
    ...
    {
      detail::zip_increment(_iterators,
                            make_integer_sequence<u64, sizeof...(Iterators)>());
      return *this;
    }

    Zip_Iterator& operator--()
    {
      detail::zip_decrement(_iterators,
                            make_integer_sequence<u64, sizeof...(Iterators)>());
      return *this;
    }
    ...
    {
      apply(_iterators,
            [n](auto&&... iterators) -> void { ((iterators += n), ...); });
      return *this;
    }
    ...
    [[nodiscard]] friend Zip_Iterator operator+(difference_type n,
                                                Zip_Iterator const& a)
    {
    ...
    {
      return detail::zip_dereference<reference>(
        _iterators, make_integer_sequence<u64, sizeof...(Iterators)>());
    }

    [[nodiscard]] reference operator[](difference_type n) const
    {
      return apply(_iterators, [n](auto&&... iterators) -> reference {
        return {*(iterators + n)...};
      });
    }
    ...
    //
    [[nodiscard]] ANTON_FORCEINLINE bool
    operator==(Zip_Iterator const& rhs) const
    {
      return detail::zip_equal(
