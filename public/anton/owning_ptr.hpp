...
namespace anton {
  ...
  // An owning pointer might manage no object, i.e. was default constructed or
  // the pointer was set to nullptr.
  // The object is destroyed using either a user-supplied deleter or the delete
  // operator if the user did not provide a deleter. Deleter is a function
  // pointer. Stateful deleters are not supported.
  //
  template<typename T>
  struct Owning_Ptr {
  public:
  ...
  public:
    ...
    Owning_Ptr(T* pointer, deleter_type deleter)
      : _pointer(pointer), _deleter(deleter)
    {
    }

    template<typename... Args>
    Owning_Ptr(Variadic_Construct_Tag tag, Args&&... args)
      : _pointer(new_obj<T>(ANTON_FWD(args)...))
    {
    }

    template<typename... Args>
    Owning_Ptr(deleter_type deleter, Variadic_Construct_Tag tag, Args&&... args)
      : _pointer(new_obj<T>(ANTON_FWD(args)...)), _deleter(deleter)
    {
    }
    ...
    Owning_Ptr(Owning_Ptr&& other)
      : _pointer(other._pointer), _deleter(other._deleter)
    {
  ...
  template<typename T>
  [[nodiscard]] bool operator==(Owning_Ptr<T> const& lhs,
                                Owning_Ptr<T> const& rhs)
  {
