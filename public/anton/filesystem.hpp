...
namespace anton::fs {
  ...
  [[nodiscard]] String concat_paths(Memory_Allocator* allocator,
                                    String_View path1, String_View path2);
  ...
  //
  // Returns:
  // A string containing the characters after the last directory or volume
  // separator.
  //
  ...
  // Returns:
  // A string containing the characters after the last directory or volume
  // separator exlcuding the last period and the characters following it.
  //
  ...
  //
  // Returns:
  // The extension including the period "." of the specified path or an empty
  // view if path does not have an extension.
  //
  ...
  //
  // Returns:
  // Returns a string consisting of all characters in path up to, but not
  // including, the last directory separator. If the path consists of a root
  // directory, e.g. "C:/", the returned string is empty.
  //
  ...
  //
  // Returns:
  // Time of the last modification of the file in milliseconds since 00:00:00
  // 1970-01-01 or -1 if the function failed to retrieve the information.
  //
  ...
  // Only the final directory will be created. Intermediate directories will not
  // be created and if they do not exist, the function will fail.
  //
  // Parameters:
  ...
  // Enumerates all immediate (i.e. non-recursively) child directories within
  // the directory identified by path. The names of the directories are not
  // prefixed with the parent path. Does not include the current directory (".")
  ...
  // enumerate_files
  // Enumerates all immediate (i.e. non-recursively) child files within the
  // directory identified by path. The names of the files are not prefixed with
  // the parent path.
  //
  // Parameters:
  // path - directory to enumerate. The path must not end in a slash ('/' or
