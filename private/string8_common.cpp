...
namespace anton {
  ...
  UTF8_Char_Iterator::UTF8_Char_Iterator(char8 const* p, i64 offset)
    : _data(p), _offset(offset)
  {
  }
  ...
  UTF8_Char_Iterator& UTF8_Char_Iterator::operator++()
  {
    if(_offset >= 0) {
      ...
      u32 const byte_count =
        unicode::get_byte_count_from_utf8_leading_byte(leading_byte);
      _data += byte_count;
      _offset += byte_count;
      return *this;
    } else {
  ...
  UTF8_Bytes::UTF8_Bytes(value_type* first, value_type* last)
  ...
  UTF8_Const_Bytes::UTF8_Const_Bytes(value_type const* first,
  ...
  UTF8_Chars::UTF8_Chars(value_type const* first, value_type const* last)
    : _begin(first), _end(last)
  {
  }
