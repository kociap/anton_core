...
namespace anton {
  ...
  struct Is_Any_Of
    : conditional<(... || is_same<T, Types>), True_Type, False_Type> {};
  ...
  struct Is_Copy_Assignable
    : Bool_Constant<__is_assignable(add_lvalue_reference<T>,
                                    add_lvalue_reference<T>)> {};

  template<typename T>
  constexpr bool is_copy_assignable =
    __is_assignable(add_lvalue_reference<T>, add_lvalue_reference<T>);
  ...
  constexpr bool is_move_assignable =
    __is_assignable(add_lvalue_reference<T>, add_rvalue_reference<T>);

  // Is_Trivially_Assignable
  //
  template<typename To, typename From>
  struct Is_Trivially_Assignable
    : Bool_Constant<__is_trivially_assignable(To, From)> {};
  ...
  struct Is_Trivially_Copy_Assignable
    : Bool_Constant<__is_trivially_assignable(add_lvalue_reference<T>,
                                              add_lvalue_reference<T>)> {};

  template<typename T>
  constexpr bool is_trivially_copy_assignable =
    __is_trivially_assignable(add_lvalue_reference<T>, add_lvalue_reference<T>);
  ...
  constexpr bool is_trivially_move_assignable =
    __is_trivially_assignable(add_lvalue_reference<T>, add_rvalue_reference<T>);
  ...
  struct Is_Copy_Constructible
    : Bool_Constant<__is_constructible(T, add_lvalue_reference<T const>)> {};

  template<typename T>
  constexpr bool is_copy_constructible =
    __is_constructible(T, add_lvalue_reference<T const>);

  // Is_Move_Constructible
  //
  template<typename T>
  struct Is_Move_Constructible
    : Bool_Constant<__is_constructible(T, add_rvalue_reference<T>)> {};

  template<typename T>
  constexpr bool is_move_constructible =
    __is_constructible(T, add_rvalue_reference<T>);
  ...
  struct Is_Trivially_Copy_Constructible
    : Bool_Constant<__is_trivially_constructible(
        T, add_lvalue_reference<T const>)> {};

  template<typename T>
  constexpr bool is_trivially_copy_constructible =
    __is_trivially_constructible(T, add_lvalue_reference<T const>);

  // Is_Trivially_Move_Constructible
  //
  template<typename T>
  struct Is_Trivially_Move_Constructible
    : Bool_Constant<__is_trivially_constructible(T, add_rvalue_reference<T>)> {
  };

  template<typename T>
  constexpr bool is_trivially_move_constructible =
    __is_trivially_constructible(T, add_rvalue_reference<T>);
  ...
  struct Is_Implicitly_Default_Constructible<
    T, void_sink<decltype(detail::check_implicit_default_construct<T>({}))>>
    : True_Type {};

  template<typename T>
  constexpr bool is_implicitly_default_constructible =
  ...
  namespace detail {
    template<typename From, typename To,
             bool = disjunction<Is_Void<To>, Is_Array<To>, Is_Function<To>>>
    struct Is_Convertible_Helper {
    private:
      template<typename F, typename T,
               typename = enable_if<is_void<F> && is_void<T>>>
      static True_Type test(int);
  ...
  namespace detail {
    ...
    //
    template<typename T, bool = disjunction<Is_Void<T>, Is_Unbounded_Array<T>,
                                            Is_Function<T>>>
...
#else
  // GCC provides __has_trivial_destructor instead of
  ...
  // Is_Signed
  template<typename T, bool arithmetic = is_arithmetic<T>,
           bool integral = is_integral<T>>
