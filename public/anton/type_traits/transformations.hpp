...
namespace anton {
    ...
    namespace detail {
        ...
        template<typename T>
        struct Add_Reference<T, void_sink<T&>> {
            using lvalue = T&;
            using rvalue = T&&;
        };
    } // namespace detail
    ...
    private:
        template<typename U>
        static auto test(int) -> Type_Identity<remove_reference<T>*>;

        template<typename U>
        static auto test(...) -> Type_Identity<T>;
    ...
    public:
        using type = conditional<detail::Is_Array<U>::value, remove_extent<U>*, conditional<is_function<T>, add_pointer<T>, remove_const<T>>>;
    };
