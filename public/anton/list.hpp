...
namespace anton {
  ...
  // List
  // List is a node-based container that guarantees address stability of the
  // elements it owns. It supports constant time insertion and removal from
  // anywhere inside the container, but does not allow random access. Provides
  // bidirectional iteration over the elements. An iterator is only invalidated
  // when its corresponding element is deallocated. Operations such as adding,
  // relocating or removing performed on elements within the list do not
  // invalidate the iterators or references to unaffected elements.
  //
  // Allocators are copied/moved/swapped during construction/assignment/swap
  // operations. When the allocators do not compare equal, all elements are
  // copied and constructed using the destination allocator.
  //
  ...
  private:
    ...
    static_assert(alignof(node_type) == alignof(Internal_Node),
                  "List_Node<T> and Internal_Node alignments differ");
  ...
  template<typename T, typename Allocator>
  List<T, Allocator>::List(allocator_type const& allocator)
  ...
  template<typename T, typename Allocator>
  List<T, Allocator>::List(List const& other, allocator_type const& allocator)
    : _allocator(allocator)
  {
    node_type* const internal_node = (node_type*)&_internal_node;
    internal_node->next = internal_node;
  ...
  template<typename T, typename Allocator>
  List<T, Allocator>::List(List&& other, allocator_type const& allocator)
    : _allocator(allocator)
  {
    node_type* const internal_node = (node_type*)&_internal_node;
    internal_node->next = internal_node;
  ...
  List<T, Allocator>::List(Variadic_Construct_Tag, Args&&... args)
  {
    ...
    ((new_node =
        (node_type*)_allocator.allocate(sizeof(node_type), alignof(node_type)),
      construct(new_node, ANTON_FWD(args)), // Construct a new node
      new_node->prev = node,
      node->next = new_node, // Insert the new node into the list
      node = new_node),
     ...);
  ...
  template<typename T, typename Allocator>
  template<typename Input_Iterator>
  List<T, Allocator>::List(Range_Construct_Tag, Input_Iterator first,
                           Input_Iterator last)
    : List(range_construct, first, last, allocator_type())
  {
  }
  ...
  template<typename Input_Iterator>
  List<T, Allocator>::List(Range_Construct_Tag, Input_Iterator first,
                           Input_Iterator last, allocator_type const& allocator)
    : _allocator(allocator)
  {
    node_type* const internal_node = (node_type*)&_internal_node;
    internal_node->next = internal_node;
  ...
  template<typename T, typename Allocator>
  template<typename... Args>
  auto List<T, Allocator>::emplace(const_iterator position,
                                   Args&&... args) -> iterator
  {
  ...
  template<typename T, typename Allocator>
  template<typename Input_Iterator>
  auto List<T, Allocator>::insert(const_iterator position, Input_Iterator first,
  ...
  void List<T, Allocator>::erase(const_iterator position)
  {
    if constexpr(ANTON_ITERATOR_DEBUG) {
      ANTON_FAIL(position.node != (node_type*)&_internal_node,
                 "cannot erase end node");
    }
  ...
  {
    node_type* const internal_node = (node_type*)&_internal_node;
    ANTON_ASSERT(internal_node->next != internal_node,
