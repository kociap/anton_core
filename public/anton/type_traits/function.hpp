...
#define ANTON_DEFINE_CALL_QUALIFIER(CLASS, CONST_OPT, REF_OPT, NOEXCEPT_OPT) \
  CLASS(/* CALL */, CONST_OPT, REF_OPT, NOEXCEPT_OPT)
...
namespace anton {
  ...
  // Removes all function qualifiers (const, &, && and noexcept) from a function
  // type.
  //
  template<typename T>
...
#define ANTON_DEFINE_TRAIT_REMOVE_FUNCTION_QUALIFIERS(                \
  CALL_QUALIFIER, CONST_QUALIFIER, REF_QUALIFIER, NOEXCEPT_QUALIFIER) \
  template<typename R, typename... Params>                            \
  struct Remove_Function_Qualifiers<R CALL_QUALIFIER(                 \
    Params...) CONST_QUALIFIER REF_QUALIFIER NOEXCEPT_QUALIFIER> {    \
    using type = R(Params...);                                        \
  };
  ANTON_DEFINE_CALL_CONST_REF_NOEXCEPT_QUALIFIERS(
    ANTON_DEFINE_TRAIT_REMOVE_FUNCTION_QUALIFIERS)
#undef ANTON_DEFINE_TRAIT_REMOVE_FUNCTION_QUALIFIERS

#define ANTON_DEFINE_TRAIT_REMOVE_ELLIPSIS_FUNCTION_QUALIFIERS(         \
  CONST_QUALIFIER, REF_QUALIFIER, NOEXCEPT_QUALIFIER)                   \
  /* No calling convention for ellipsis */                              \
  template<typename R, typename... Params>                              \
  struct Remove_Function_Qualifiers<R(                                  \
    Params..., ...) CONST_QUALIFIER REF_QUALIFIER NOEXCEPT_QUALIFIER> { \
    using type = R(Params..., ...);                                     \
  };
  ANTON_DEFINE_CONST_REF_NOEXCEPT_QUALIFIERS(
    ANTON_DEFINE_TRAIT_REMOVE_ELLIPSIS_FUNCTION_QUALIFIERS)
...
#define ANTON_DEFINE_TRAIT_IS_FUNCTION(CALL_QUALIFIER, CONST_QUALIFIER,   \
                                       REF_QUALIFIER, NOEXCEPT_QUALIFIER) \
  template<typename R, typename... Params>                                \
  struct Is_Function<R CALL_QUALIFIER(Params...)                          \
                       CONST_QUALIFIER REF_QUALIFIER NOEXCEPT_QUALIFIER>  \
    : True_Type {};
  ANTON_DEFINE_CALL_CONST_REF_NOEXCEPT_QUALIFIERS(
    ANTON_DEFINE_TRAIT_IS_FUNCTION)
#undef ANTON_DEFINE_TRAIT_IS_FUNCTION

#define ANTON_DEFINE_TRAIT_IS_FUNCTION_ELLIPSIS(                         \
  CONST_QUALIFIER, REF_QUALIFIER, NOEXCEPT_QUALIFIER)                    \
  template<typename R, typename... Params>                               \
  struct Is_Function<R(Params..., ...)                                   \
                       CONST_QUALIFIER REF_QUALIFIER NOEXCEPT_QUALIFIER> \
    : True_Type {};
  ANTON_DEFINE_CONST_REF_NOEXCEPT_QUALIFIERS(
    ANTON_DEFINE_TRAIT_IS_FUNCTION_ELLIPSIS)
#undef ANTON_DEFINE_TRAIT_IS_FUNCTION_ELLIPSIS
  ...
  namespace detail {
    ...
    private:
      ...
      static type_sink<True_Type, decltype(declval<XFn>()(declval<XArgs>()...))>
      test(int);
  ...
  struct Invoke_Result
    : enable_if<detail::Is_Invocable<Fn, Args...>::value,
  ...
  struct Is_Invocable_R
    : Bool_Constant<conjunction<
        Is_Invocable<Fn, Args...>,
        detail::Is_Same_Type<Identity<Return>, Invoke_Result<Fn, Args...>>>> {};
  ...
  namespace detail {
    ...
    struct Reflect_Function
      : Reflect_Function_Args<sizeof...(Arguments), Arguments...> {
      using return_type = Return_Type;
    };

    template<typename Return_Type, typename... Arguments>
    auto fn_refl_decay(Return_Type (*)(Arguments...))
      -> Reflect_Function<Return_Type, Arguments...>;

    template<typename Class, typename Return_Type, typename... Arguments>
    auto fn_refl_decay(Return_Type (Class::*)(Arguments...))
      -> Reflect_Function<Return_Type, Arguments...>;

    template<typename Class, typename Return_Type, typename... Arguments>
    auto fn_refl_decay(Return_Type (Class::*)(Arguments...)
                         const) -> Reflect_Function<Return_Type, Arguments...>;
  ...
  // Reflect_Function
  // Function reflection trait that allows to access the return type and
  // arguments of a callable. Requires that Fn be a function pointer, a member
  // function pointer, a functor or a non-generic lambda.
  //
  template<typename T>
  struct Reflect_Function
    : decltype(detail::fn_refl_decay(anton::declval<T>())){};
} // namespace anton
