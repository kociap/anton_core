...
namespace anton {
  namespace detail {
    ...
    struct Tuple_Base<Integer_Sequence<u64, Indices...>, Types...>
      : public Tuple_Child<Indices, Types>... {
    public:
      constexpr Tuple_Base() = default;

      template<
        typename... Args,
        enable_if<sizeof...(Args) == sizeof...(Types) && (sizeof...(Args) > 0),
                  int> = 0>
      constexpr Tuple_Base(Args&&... args)
        : Tuple_Child<Indices, Types>(ANTON_FWD(args))...
      {
      }
    };
  } // namespace detail
  ...
  using Tuple =
    detail::Tuple_Base<make_integer_sequence<u64, sizeof...(Ts)>, Ts...>;
  ...
  template<typename... Types>
  struct Tuple_Size<Tuple<Types...> const>
  ...
  struct Tuple_Element<Index, Tuple<Types...>> {
    using type =
      typename detail::Tuple_Element<Index, 0, Tuple<Types...>>::type;
  };

  template<u64 Index, typename... Types>
  struct Tuple_Element<Index, Tuple<Types...> const> {
    using type = add_const<
      typename detail::Tuple_Element<Index, 0, Tuple<Types...>>::type>;
  };
  ...
  constexpr tuple_element<Index, Tuple<Types...>>& get(Tuple<Types...>& t)
  {
    ...
    return static_cast<type&>(
      static_cast<detail::Tuple_Child<Index, type>&>(t).value);
  }

  template<u64 Index, typename... Types>
  constexpr tuple_element<Index, Tuple<Types...>> const&
  get(Tuple<Types...> const& t)
  {
    using type = tuple_element<Index, Tuple<Types...>>;
    return static_cast<type const&>(
      static_cast<detail::Tuple_Child<Index, type> const&>(t).value);
  }
  ...
  // apply
  // Invokes a callable object with all elements of a tuple passed as the
  // arguments.
  //
  // Parameters:
  //    tuple - the tuple whose elements to pass to the callable.
  // callable - callable to be invoked. Must have the signature
  //            R(T1, T2, T3, ...) where each TX matches the type in the tuple
  //            at position X.
  //
  // Returns:
  ...
  {
    return detail::apply(
      ANTON_FWD(tuple), ANTON_FWD(callable),
      make_integer_sequence<u64, tuple_size<remove_reference<Tuple>>>());
  }
} // namespace anton

// We provide std::tuple_size and std::tuple_element to enable structured
// bindings.
namespace std {
  template<typename... Types>
  struct tuple_size<anton::Tuple<Types...>>
    : anton::Tuple_Size<anton::Tuple<Types...>> {};

  template<typename... Types>
  struct tuple_size<anton::Tuple<Types...> const>
    : anton::Tuple_Size<anton::Tuple<Types...> const> {};

  template<anton::usize I, typename... Types>
  struct tuple_element<I, anton::Tuple<Types...>>
    : anton::Tuple_Element<I, anton::Tuple<Types...>> {};

  template<anton::usize I, typename... Types>
  struct tuple_element<I, anton::Tuple<Types...> const>
    : anton::Tuple_Element<I, anton::Tuple<Types...> const> {};
} // namespace std
