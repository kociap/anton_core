...
namespace anton {
  ...
  struct Fixed_Array {
    static_assert(Capacity > 0,
                  "Fixed_Array's capacity must be greater than 0");
...
namespace anton {
  ...
  Fixed_Array<T, Capacity>::Fixed_Array(size_type const s): _size(s)
  {
    ...
    anton::uninitialized_default_construct_n(reinterpret_cast<T*>(_data),
                                             _size);
  }

  template<typename T, i64 Capacity>
  Fixed_Array<T, Capacity>::Fixed_Array(size_type const s, value_type const& v)
    : _size(s)
  {
    ANTON_VERIFY(s <= Capacity, u8"size is greater than capacity");
  ...
  template<typename T, i64 Capacity>
  Fixed_Array<T, Capacity>::Fixed_Array(Fixed_Array const& other)
    : _size(other._size)
  {
    anton::uninitialized_copy_n(reinterpret_cast<T const*>(other._data), _size,
  ...
  {
    anton::uninitialized_move_n(reinterpret_cast<T*>(other._data), _size,
                                reinterpret_cast<T*>(_data));
  }

  template<typename T, i64 Capacity>
  template<typename Input_Iterator>
  Fixed_Array<T, Capacity>::Fixed_Array(Range_Construct_Tag,
                                        ...
                                        Input_Iterator last)
  {
    ...
    ANTON_VERIFY(
      distance <= Capacity,
      u8"distance between last and first is greater than the capacity");
    anton::uninitialized_copy(first, last, reinterpret_cast<T*>(_data));
  }

  template<typename T, i64 Capacity>
  template<typename... Args>
  Fixed_Array<T, Capacity>::Fixed_Array(Variadic_Construct_Tag, Args&&... args)
    : _size(sizeof...(Args))
  {
    static_assert(
      sizeof...(Args) <= Capacity,
      "attempting to construct Fixed_Array with more elements than capacity");
    anton::uninitialized_variadic_construct(reinterpret_cast<T*>(_data),
                                            ANTON_FWD(args)...);
  }
  ...
  template<typename T, i64 Capacity>
  Fixed_Array<T, Capacity>&
  Fixed_Array<T, Capacity>::operator=(Fixed_Array const& other)
  {
#define PTR(data, offset) reinterpret_cast<T*>(data + offset)
#define PTRC(data, offset) reinterpret_cast<T const*>(data + offset)
    if(_size >= other._size) {
      anton::copy(PTRC(other._data, 0), PTRC(other._data, other._size),
                  PTR(_data, 0));
      anton::destruct_n(PTR(_data, other._size), _size - other._size);
      _size = other._size;
    } else {
      anton::copy(PTRC(other._data, 0), PTRC(other._data, _size),
                  PTR(_data, 0));
      anton::uninitialized_copy(PTRC(other._data, _size),
                                PTRC(other._data, other._size),
                                PTR(_data, _size));
      _size = other._size;
    }
    return *this;
#undef PTR
  ...
  template<typename T, i64 Capacity>
  Fixed_Array<T, Capacity>&
  Fixed_Array<T, Capacity>::operator=(Fixed_Array&& other)
  {
#define PTR(data, offset) reinterpret_cast<T*>(data + offset)
    if(_size >= other._size) {
      anton::move(PTR(other._data, 0), PTR(other._data, other._size),
                  PTR(_data, 0));
      anton::destruct_n(PTR(_data, other._size), _size - other._size);
      _size = other._size;
    } else {
      anton::move(PTR(other._data, 0), PTR(other._data, _size), PTR(_data, 0));
      anton::uninitialized_move(PTR(other._data, _size),
                                PTR(other._data, other._size),
                                PTR(_data, _size));
      _size = other._size;
    }
    return *this;
#undef PTR
  ...
  {
    ANTON_VERIFY(s <= Capacity && s >= 0,
                 u8"requested size was outside the range [0, capacity()]");
    if(s >= _size) {
      anton::uninitialized_default_construct_n(
        reinterpret_cast<T*>(_data + _size), s - _size);
    } else {
  ...
  void Fixed_Array<T, Capacity>::resize(size_type const s, T const& v)
  {
    ANTON_VERIFY(s <= Capacity && s >= 0,
                 u8"requested size was outside the range [0, capacity()]");
    if(s >= _size) {
      anton::uninitialized_fill_n(reinterpret_cast<T*>(_data + _size),
                                  s - _size, v);
    } else {
      anton::destruct_n(reinterpret_cast<T*>(_data + s), _size - s);
    }
  ...
  {
    ANTON_VERIFY(_size < Capacity,
                 u8"cannot emplace_back element in a full Fixed_Array");
  ...
  {
    ANTON_VERIFY(_size > 0,
                 u8"trying to pop an element from an empty Fixed_Array");
