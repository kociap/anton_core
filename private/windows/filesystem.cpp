...
namespace anton::fs {
  [[nodiscard]] static Array<char16>
  string8_to_string16(String_View const string8)
  {
    i64 const string16_bytes = unicode::convert_utf8_to_utf16(
      string8.data(), string8.size_bytes(), nullptr);
    // Add 1 to the length for null-terminator.
    Array<char16> string16(1 + string16_bytes / sizeof(char16), 0);
    unicode::convert_utf8_to_utf16(string8.data(), string8.size_bytes(),
                                   string16.data());
    return string16;
  }

  [[nodiscard]] static String
  fs_path_to_string(std::filesystem::path const& path)
  {
  ...
  [[nodiscard]] static String
  fs_path_to_string(Memory_Allocator* allocator,
  ...
  String concat_paths(Memory_Allocator* allocator, String_View const lhs,
  ...
  String make_relative(String_View path, String_View base_path)
  {
    ...
    std::filesystem::path b(
      std::string_view(base_path.data(), base_path.size_bytes()));
  ...
  i64 get_last_write_time(String_View path)
  {
    ...
    // Open file for reading, allow other processes to open for reading, must
    // exist.
    HANDLE const file_handle =
      CreateFileW((wchar_t const*)wpath.data(), GENERIC_READ, FILE_SHARE_READ,
    ...
    i64 last_write_time_64 = (i64)last_write_time.dwHighDateTime << 32 |
                             (i64)last_write_time.dwLowDateTime;
    // Time is reported since 00:00:00 1601-01-01. We want to adjust it so that
    // it starts at 00:00:00 1970-01-01.
    ...
    // Clamp time to be in range 00:00:00 1970-01-01 to present because we don't
    // care about files older than that.
  ...
  bool copy_file(String_View source, String_View destination, bool overwrite)
  {
    ...
    BOOL cancel = false;
    // We set the NO_BUFFERING flag to support large files, since the microsoft
    // documentation says that it's recommended for large files.
    ...
    bool const result = CopyFileEx((wchar_t const*)source16.data(),
                                   (wchar_t const*)destination16.data(),
                                   nullptr, nullptr, &cancel, flags);
    return result;
  }
  ...
  {
    // We have to append the wildcard character at the end /* to search the
    // directory.
    ...
    HANDLE find_handle =
      FindFirstFileExW((wchar_t const*)wpath.data(), FindExInfoBasic, &data,
    ...
    while(true) {
      ...
      if(is_directory) {
        ...
        bool const is_current_directory =
          data.cFileName[0] == '.' && data.cFileName[1] == '\0';
        bool const is_parent_directory = data.cFileName[0] == '.' &&
                                         data.cFileName[1] == '.' &&
                                         data.cFileName[2] == '\0';
