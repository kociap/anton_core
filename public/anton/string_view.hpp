...
namespace anton {
  ...
  public:
    ...
    constexpr String_View(value_type const* str, size_type size)
      : _begin(str), _end(str + size)
    {
      if constexpr(ANTON_STRING_VIEW_VERIFY_ENCODING) {
        // TODO: Implement
      }
    ...
    constexpr String_View(value_type const* first, value_type const* last)
    ...
    String_View(char_iterator first, char_iterator last)
      : _begin(first.get_underlying_pointer()),
        _end(last.get_underlying_pointer())
    {
    }
  ...
  inline namespace literals {
    [[nodiscard]] constexpr String_View operator""_sv(char8 const* literal,
                                                      u64 size)
    {
  ...
  //
  // Returns:
  // The start position of the substring within string or npos if the substring
  // is not present.
  //
  [[nodiscard]] constexpr i64 find_substring(String_View const string,
                                             String_View const substr)
  {
    // Bruteforce
    char8 const* const string_data = string.data();
    char8 const* const substr_data = substr.data();
    for(i64 i = 0, end = string.size_bytes() - substr.size_bytes(); i <= end;
        ++i) {
      bool equal = true;
  ...
  // is not present.
  //
  [[nodiscard]] constexpr i64 find_last_substring(String_View const string,
                                                  String_View const substr)
  {
    // Bruteforce
    char8 const* const string_data = string.data();
  ...
  //
  [[nodiscard]] constexpr bool begins_with(String_View const string,
                                           String_View const substr)
  {
    ...
    for(; str_begin != str_end && substr_begin != substr_end;
        ++str_begin, ++substr_begin) {
  ...
  //
  [[nodiscard]] constexpr bool ends_with(String_View const string,
                                         String_View const substr)
  {
    ...
    for(; str_end != str_begin && substr_end != substr_begin;
        --str_begin, --substr_begin) {
  ...
  shrink_front_bytes(String_View const string, i64 const count)
  {
  ...
  //
  [[nodiscard]] constexpr String_View
  shrink_back_bytes(String_View const string, i64 const count)
  {
    ANTON_ASSERT(count >= 0, "shrink count must be positive");
    i64 const shrink_size = math::min(string.size_bytes(), count);
  ...
  // respectively.
  //
  [[nodiscard]] constexpr String_View shrink_bytes(String_View const string,
                                                   i64 const front_count,
                                                   i64 const back_count)
  {
    ...
    return shrink_back_bytes(shrink_front_bytes(string, front_count),
                             back_count);
  }
