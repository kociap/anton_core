...
// We need SymTagEnum and this define allows us to access it without including
// cvconst.h (???)
#define _NO_CVCONST_H
#include <Windows.h>
// Windows.h must be included before DbgHelp.h. Otherwise we get undefined
// identifiers in DbgHelp.h
#include <DbgHelp.h>
#include <debugapi.h>

// TODO: add locking to ensure thread safety because all the functions are
// singlethreaded
...
namespace anton::stacktrace {
  ...
  Array<Frame_Record*> capture_call_stack()
  {
    ...
    i64 const captured_frames =
      RtlCaptureStackBackTrace(1, 1024, (void**)records.data(), nullptr);
    records.force_size(captured_frames);
    return records;
  }

  [[nodiscard]] static String
  get_type_as_string(HANDLE process, ULONG64 module_base, ULONG64 index);

  [[nodiscard]] static String process_base_type(HANDLE const process,
                                                ULONG64 const module_base,
                                                ULONG64 const index)
  {
    ULONG64 length = 0;
    ...
    DWORD base_type = 0;
    if(!SymGetTypeInfo(process, module_base, index, TI_GET_BASETYPE,
                       &base_type)) {
      return u8"unknown_type"_s;
    }
  ...
  [[nodiscard]] static String process_typedef(HANDLE const process,
                                              ULONG64 const module_base,
                                              ULONG64 const index)
  {
    if(WCHAR* typedef_name = nullptr; SymGetTypeInfo(
         process, module_base, index, TI_GET_SYMNAME, &typedef_name)) {
      String name =
        String::from_utf16(reinterpret_cast<char16_t*>(typedef_name));
  ...
  [[nodiscard]] static String process_pointer_type(HANDLE const process,
                                                   ULONG64 const module_base,
                                                   ULONG64 const index)
  {
    // SymGetTypeInfo writes a DWORD instead of the documented boolean when the
    // GetType parameter is TI_GET_IS_REFERENCE.
    DWORD is_reference = false;
    if(!SymGetTypeInfo(process, module_base, index, TI_GET_IS_REFERENCE,
                       &is_reference)) {
      return u8"unknown_type"_s;
    }

    if(DWORD type_index = 0; SymGetTypeInfo(process, module_base, index,
                                            TI_GET_TYPEID, &type_index)) {
  ...
  [[nodiscard]] static String process_function_type(HANDLE const process,
                                                    ULONG64 const module_base,
                                                    ULONG64 const index,
                                                    String_View name)
  {
    String type_string;
    // Return type
    if(DWORD return_type_index = 0; SymGetTypeInfo(
         process, module_base, index, TI_GET_TYPEID, &return_type_index)) {
      String const return_type =
        get_type_as_string(process, module_base, return_type_index);
    ...
    if(!SymGetTypeInfo(process, module_base, index, TI_GET_CHILDRENCOUNT,
                       &parameters_count)) {
      type_string += u8"()"_sv;
      return type_string;
    }

    u64 const alloc_size =
      sizeof(TI_FINDCHILDREN_PARAMS) + sizeof(ULONG) * parameters_count;
    Owning_Ptr<void> children_params_memory =
      allocate(alloc_size, alignof(TI_FINDCHILDREN_PARAMS));
    TI_FINDCHILDREN_PARAMS& children_params =
      *reinterpret_cast<TI_FINDCHILDREN_PARAMS*>(children_params_memory.get());
    memset(&children_params, 0, alloc_size);
    children_params.Count = parameters_count;
    if(!SymGetTypeInfo(process, module_base, index, TI_FINDCHILDREN,
                       &children_params)) {
      type_string += u8"()"_sv;
      return type_string;
    }
    ...
    for(u64 i = children_params.Start; i < children_params.Count; ++i) {
      String const argument =
        get_type_as_string(process, module_base, children_params.ChildId[i]);
  ...
  [[nodiscard]] static String process_function(HANDLE const process,
                                               ULONG64 const module_base,
                                               ULONG64 const index)
  {
    DWORD type_index = 0;
    if(!SymGetTypeInfo(process, module_base, index, TI_GET_TYPEID,
                       &type_index)) {
      return u8"unknown_function"_s;
    }

    WCHAR* fn_name_wchar = nullptr;
    bool const success = SymGetTypeInfo(process, module_base, index,
                                        TI_GET_SYMNAME, &fn_name_wchar);
    // For some reason SymGetTypeInfo may return true, but still not return a
    // valid pointer in fn_name_wchar. We must guard against that undocumented
    // behaviour with a nullptr check.
    if(success && fn_name_wchar != nullptr) {
      String function_name =
        String::from_utf16(reinterpret_cast<char16_t*>(fn_name_wchar));
      LocalFree(fn_name_wchar);
      return process_function_type(process, module_base, type_index,
                                   function_name);
    } else {
      return process_function_type(process, module_base, type_index,
                                   u8"unknown_name"_sv);
  ...
  [[nodiscard]] static String
  process_function_argument(HANDLE const process, ULONG64 const module_base,
                            ULONG64 const index)
  {
    if(DWORD type_index = 0; SymGetTypeInfo(process, module_base, index,
                                            TI_GET_TYPEID, &type_index)) {
  ...
  [[nodiscard]] static String process_class(HANDLE const process,
                                            ULONG64 const module_base,
                                            ULONG64 const index)
  {
    if(WCHAR* name = nullptr;
       SymGetTypeInfo(process, module_base, index, TI_GET_SYMNAME, &name)) {
  ...
  [[nodiscard]] static String process_udt(HANDLE const process,
                                          ULONG64 const module_base,
                                          ULONG64 const index)
  {
    ...
    if(DWORD udt_kind = ~0;
       SymGetTypeInfo(process, module_base, index, TI_GET_UDTKIND, &udt_kind)) {
      switch(udt_kind) {
      case UdtStruct:
      case UdtClass:
      case UdtUnion:
        // Since we are only getting the name of the UDT, all 3 cases are the
        // same.
  ...
  static String get_type_as_string(HANDLE const process,
                                   ULONG64 const module_base,
                                   ULONG64 const index)
  {
    DWORD type_tag = 0;
  ...
  static String get_record_function_name(Frame_Record* record)
  {
    ...
    DWORD type_tag = 0;
    if(!SymGetTypeInfo(process_handle, symbol_info.ModBase, symbol_info.Index,
    ...
    return process_function(process_handle, symbol_info.ModBase,
                            symbol_info.Index);
  }
  ...
  static i64 get_record_line(Frame_Record* record)
  {
    ...
    if(DWORD displacement = 0; SymGetLineFromAddr64(
         process_handle, DWORD64(record), &displacement, &image_line)) {
