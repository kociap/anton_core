...
namespace anton {
    template<typename T>
    [[deprecated]] [[nodiscard]] constexpr T&& forward(remove_reference<T>& v) {
    ...
    template<typename T>
    [[deprecated]] [[nodiscard]] constexpr remove_reference<T>&& move(T&& v) {
        return static_cast<remove_reference<T>&&>(v);
    }

    template<typename T>
    [[deprecated]] [[nodiscard]] constexpr add_const<T>& as_const(T& v) {
        return v;
    }

    template<typename T>
    [[deprecated]] void as_const(T const&&) = delete;
} // namespace anton
