...
namespace anton {
    ...
    auto zip(Rangelike&&... rangelike) {
        static_assert(sizeof...(rangelike) > 1, "zip requires at least 2 range-like objects");
        return Range(Zip_Iterator(rangelike.begin()...), Zip_Iterator(rangelike.end()...));
    }

    template<typename Rangelike>
    auto enumerate(Rangelike&& rangelike) {
        return zip(Range(Enumerate_Iterator(0), Enumerate_Iterator(limits::maximum_i64)), rangelike);
    }
} // namespace anton
